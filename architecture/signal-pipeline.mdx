...
For `effect=mutation` it calls `apply_signal_mutation`, which feeds the proposal through the legacy `apply_state_change` (v1) writer after a confidence gate *(signal_mutations.py)*.
...
2. Score each instinct against the signal via `_score_keyword_overlap` — weighted: 0.30 domain + 0.30 keywords + 0.15 input_type + 0.15 attachment + 0.10 tags. As of Gap 5b (2026-05-24), matching is **substring + tokenised**, not set-intersection on single-word tokens — multi-word patterns now actually match *(packages/learn/src/matching/scorer.py)*.
...
A second write path — the legacy `/admin/needs-attention/:id/{done,dispatch,skip}` surface — also mints a mirror decision via `mintDecisionMirror`, **unless** the body carries `decision_origin` (which DecisionRouter sets when *it* calls `/dispatch`). This guard prevents the recursive loop where DecisionRouter→/dispatch→mintDecisionMirror→DecisionRouter again *(packages/ctrl/src/api/routes/attention.ts)*.
