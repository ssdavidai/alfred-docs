...
Alfred Black runs as a single-VM stack: one git repo, one `.env` file, one `docker compose up`. Every install is the principal's own machine, end to end.
...
- [Sidecars](/architecture/sidecars) — Plane, Sure, Vaultwarden, Paperclip, Tailscale.
