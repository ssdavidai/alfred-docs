---
title: "The five-store model"
description: "Vault, alfred-state.db, ingest.db, cold archive, files — and the promotion contract that keeps them honest."
---

> The vault is the principal's published output, not the system's database.

Alfred Black persists data in **five stores**, each with a different lifecycle, a different access pattern, and a single writer. The split is intentional: markdown is the wrong shape for a high-write firehose, SQLite is the wrong shape for something the principal is meant to open in Obsidian and edit by hand, and neither is the right shape for a 2 GB PDF the accountant just sent.
...
| 5 | **files** | `files_data` + `files_cold_data` named volumes; metadata in `alfred-state.db.files` | ctrl-api | Binary blobs — PDFs, images, archives, audio — the principal owns and Alfred can read. See [Files](/surfaces/files) for the surface; dedup at upload (sha256) + 90-day zstd cold archive. |
...
<Note>
The standalone `alfred-vault` Python package (the daemon) recognises 20+ additional entity types for development and migration tooling, but ctrl-api's promotion contract enforces only these twelve in production writes.
</Note>
...
- `needs_attention/` — the Desk queue (markdown-backed)
