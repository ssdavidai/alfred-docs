...
**Writers must set both `status` and `state`.** The validator-compatible default for a fresh task is `status: todo, state: pending`. A historic comment referenced `status: queued`, which the validator rejects with HTTP 500 — don't propagate it.
...
2. **`POST /api/v1/admin/needs-attention/:id/{done,dispatch,skip}`** (legacy). Performs the flip plus the legacy `needs_attention_action` audit, and mints a mirror decision via `mintDecisionMirror` *unless* the body carries `decision_origin` (which only DecisionRouter sets — preventing recursion).
3. **Signal-router autonomous fire** with `principal: alfred` and `decision_origin: instinct_fire`.

Delegate has a fourth concern: after the 2026-05-24 work that severed signal-router's `principal_delegate_override`, DecisionRouter now calls `dispatch_action_to_agent` directly when `intent=delegate`. Three idempotency layers — state guard, `dispatching` mark, `side_effects.agent_dispatched` — prevent double-firing.
