...
`alfred-learn` is Alfred Black's **intelligence layer** *(packages/learn/src/worker.py)*. It runs as a single Temporal worker on task queue `alfred-learn` *(packages/learn/src/config.py:13)*, hosts every scheduled workflow that turns inbound streams into Desk cards and observations into instinct upgrades, and coordinates onboarding, briefing, and Plane two-way sync.
...
<Note>The package's `SPEC.md` and CLAUDE.md describe six canonical workflows. The live worker registers 36 — the original six (event processor, session tracker, briefing, learning, reflection, judgment) plus everything that has been added since (signal pipeline, decision routing, narrative composition, Plane two-way sync, Vexa meeting capture, Omi audio, chore promotion, decay watcher, reversal calibration, stream sweep, …).</Note>
...
- **Conditionally registered** — Plane sync, Vexa intake, signal extract/router, reversal calibration are only registered if their gate env var is true.
...
| MeetingCaptureWorkflow | `al-meeting-capture` | 60 s | `VEXA_ENABLED` |
| TranscriptIntakeWorkflow | `al-transcript-intake` | 60 s | `VEXA_ENABLED` |
...
| StewardWorkflow | — | per-matter (deprecated; tombstone) | — |
...
2. For each event:
   ...
   - `extract_observation_from_signal` (OBS-2, patched gate `signal_extract_obs_v1`) — match instinct via `instinct_match.best_instinct_path`, write observation.
...
2. Per signal:
   - **effect=mutation** → `apply_signal_mutation` (legacy v1 path via Steward's `apply_state_change`).
...
The brief replaces the old `DailyDigestWorkflow` / `DailyMorningBriefingWorkflow` / `DailyEveningDigestWorkflow` trio (commit f20556d).
</Accordion>
...
RFC #884 Living Narratives. Walks every active matter and asks the clerk to draft a fresh 2–4 sentence `current_state` paragraph from the last 24h of signals + task transitions, stamping `as_of: <ISO timestamp>`. Matters with zero activity in the window are skipped without invoking the clerk.
...
Workflow 4 of the canonical six. Processes the observation queue and `alfred_instructions` (manual training nudges from the principal) into observation records.
...
Workflow 6. Per-input routing decisions — when a new input arrives, decides whether it's noise, an observation candidate, or signal-worthy. Augments the deterministic signal extractor for novel input types where pattern-matching is too brittle.
...
OBS-4. Hourly deterministic scan of the unified observation pool. Clusters by `(sender, intent)` and writes `pattern_proposal` records for surviving clusters. Cheap (one ctrl-api list call + in-memory work).
...
Workflow 1 of the canonical six — zero-LLM stream event routing. Reads pending events from `ingest.db`, dispatches:
...
The curator (vault-curator agent) does the actual classification — Workflow 1 is just a dispatcher.
...
**Schedule**: `al-stream-sweep` every 2 min *(packages/learn/scripts/register_schedules.py:100-102)*. The sweep reads each stream's persisted `schedule_interval_seconds` / `last_pull_at` and dispatches `StreamPullerWorkflow` for any stream due. Replaces the legacy per-stream `al-stream-pull-<id>` schedules.
...
Workflow 2. Rolling state machine for the principal's "sessions" — conversational threads that span multiple inputs. Promotes accumulated session content to vault notes when a session closes.
...
One-way vault → Plane sync (#536 B4). Reads the tenant's vault, projects matters as Plane projects and tasks as Plane issues. Per-batch cursor advancement; 5-minute execution timeout per run.
...
## Domain 7 — Meeting capture (Vexa)

The 9-container Vexa profile that joins meetings as a bot, transcribes, and feeds actions back to the signal pipeline.

<Accordion title="MeetingCaptureWorkflow">
**Schedule**: every 60 s (`al-meeting-capture`) *(packages/learn/scripts/register_schedules.py:109-111)*.

**Gate**: `VEXA_ENABLED=true` at the docker compose profile level.

Polls the gcal stream for upcoming Meet events, dispatches the Vexa bot to join. Bot pre-joins ~5 minutes before start time.

**Activities**: `list_upcoming_meet_events`, `dispatch_vexa_bot`, `mark_meeting_dispatched`.
</Accordion>

<Accordion title="TranscriptIntakeWorkflow">
**Schedule**: every 60 s (`al-transcript-intake`) *(packages/learn/scripts/register_schedules.py:113-115)*.

Polls Vexa's webhook stream for completed transcripts. Extracts action candidates, writes them as stream events, and the signal pipeline picks them up from there.

**Activities**: `list_pending_transcripts`, `extract_action_candidates`, `write_transcript_signals`, `mark_transcript_processed`.
</Accordion>
...
**Schedule**: ad-hoc (one invocation per signup). Started by ctrl-api when Sir completes the `/composing → /preparing` ritual.

**Pipeline** *(packages/learn/src/workflows/onboarding_pipeline.py)* — replaces the 101-sequential-Clerk-call v2 with 4 direct Opus 4.6 calls in ~5 minutes:
...
## Domain 10 — Steward (legacy & sweep)

The Steward perception-and-action loop, collapsed from N per-matter schedules to a single sweep.
...
Walks every active matter once per sweep, runs the Steward evaluation loop (gather signals → evaluate state → propose mutation → apply via state-mutator). Replaces the per-matter `al-steward-<slug>` schedules that #52 collapsed.

**Activities**: `list_active_matters`, `gather_signals_for_matter`, `evaluate_matter_state`, `apply_state_change_v2`.
</Accordion>

<Accordion title="StewardWorkflow (deprecated)">
The original per-matter Steward workflow. Tombstone-registered so in-flight runs from before #52 can replay deterministically; new schedules are not created.
</Accordion>
...
#645 safety-net reaper for the persistent Composio reconnect ledger written by ctrl-api after PR #646. Ctrl-api has an in-process `setTimeout` fast path; this Temporal schedule guarantees cleanup eventually fires even if ctrl-api restarts before the in-process timer (1h grace) elapses.
...
- Default `"shadow"` — read-only audit emit, no mutation applied. *(Note: CLAUDE.md §8 documents default `"live"`; live code defaults `"shadow"`.)*
